...
#[ephemeral]
#[program]
pub mod player_powers {

    use super::*;

    pub fn initialize(ctx: Context<InitUser>, dna: u8) -> Result<()> {
        ctx.accounts.initialize(dna, &ctx.bumps)?;
        Ok(())
    }

    pub fn update(ctx: Context<UpdatePlayer>) -> Result<()> {
        ctx.accounts.update()?;
        Ok(())
    }
